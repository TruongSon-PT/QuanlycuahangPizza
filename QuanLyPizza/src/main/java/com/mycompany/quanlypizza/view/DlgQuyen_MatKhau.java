...
public class DlgQuyen_MatKhau extends javax.swing.JDialog {
    ...
    private PhanQuyenServiceImp phanQuyenBUS = new PhanQuyenServiceImp();
    private TaiKhoanServiceImp taiKhoanBUS = new TaiKhoanServiceImp();

    private void loadDataCmbQuyen() {
        txtMatKhau_MaNV.setText(maNV);
        txtQuyen_MaNV.setText(maNV);

        String tenDangNhap = taiKhoanBUS.getTenDNTheoMa(maNV);
        if (tenDangNhap == null) {
            
            MyDialogCommon dlg = new MyDialogCommon("Nhân viên này chưa có tài khoản! \n Bạn có muốn cấp tài khoản cho nhân viên này?", MyDialogCommon.ERROR_DIALOG);
            ...
            if (dlg.getAction() == MyDialogCommon.CANCEL_OPTION) {
                ...
                DlgCapTaiKhoan dlgCapTK = new DlgCapTaiKhoan(maNV);
                dlgCapTK.setVisible(true);
            }
        }
        ...
        List<PhanQuyen> dsq = phanQuyenBUS.getDanhSachQuyen();
        for (PhanQuyen pq : dsq) {
            cmbQuyen.addItem(pq.getQuyen());
        }

        String quyen = taiKhoanBUS.getQuyen(maNV);
    ...
    private void btnCapMatKhauActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCapMatKhauActionPerformed
        taiKhoanBUS.datLaiMK(maNV, txtMatKhau_TenDangNhap.getText());
